...
#    ntr=1
    ...
    FFI = np.array([ 1, 3, 6, 1, 5]) 
    
    # higher bounds of the different tested freq. bands
    FFE = np.array([ 3, 6, 9.99,5 , NyF-0.01]) 
...
#    for i in range(ntr):
    ...
    E1FFT = np.trapz(FFTsmooth_norm[0:npts/4])
    E2FFT = np.trapz(FFTsmooth_norm[npts/4-1:2*npts/4])
    E3FFT = np.trapz(FFTsmooth_norm[2*npts/4-1:3*npts/4])
    E4FFT = np.trapz(FFTsmooth_norm[3*npts/4-1:npts])
...
def get_pseudo_spectral_stuff(Data, sps):
    ...
    n = 2048 
    Freq=np.linspace(0,sps,n/2) # Sampling of frequency array
    ...
    for v in range(0,np.size(smooth_spec,1)):
        ...
        CentoiX[v]=(Freq[CentroIndex])
        CentoiX1[v]=(Freq[np.around(centeroidnpX(smooth_spec[0:CentroIndex,v]))])
        CentoiX3[v]=(Freq[np.around(centeroidnpX(smooth_spec[CentroIndex:800,v])+CentroIndex)])
