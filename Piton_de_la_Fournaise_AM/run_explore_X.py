...
best_atts = list(['RappMaxStd', 'Duration', 'RappMaxMean', 'KurtoEnv', 'RappMaxMedian',
             'AsDec'])
...
for sta in station_names:
    ...
    # histograms
    print "Plotting histograms for %d classes and %d attributes" % (len(df_list), len(best_atts))
    gr.plot_histograms(df_list, best_atts, color_list, figdir, sta)

    # scatter plots
    print "Plotting scatter plots for %d classes and %d attributes" % (len(df_list), len(best_atts))
    gr.plot_scatterplots(df_list, best_atts, color_list, figdir, sta)

    # time plots
    print "Plotting timeseries plots for %d classes and %d attributes" % (len(df_list), len(best_atts))
