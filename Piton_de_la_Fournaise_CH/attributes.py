...
def calculate_all_attributes(obspy_stream):
    ...
    KurtoEnv, KurtoSig, SkewnessEnv, SkewnessSig =\
        get_KurtoSkewStuff(obspy_stream, env)
    ...
    rectilinP, azimuthP, dipP, Plani =\
        get_polarization_stuff(obspy_stream, env)
...
def duration(obspy_stream):
    ...
    for i in xrange(ntr):
        ...
        # la duree ci-dessus est fausse...
        # peut-on changer avec celle ci-dessous ?
...
def get_AsDec(st, env):
    ...
    for i in xrange(ntr):
        ...
        RmsDecAmpEnv[i] = np.abs(np.mean(
            np.abs(hilbert(dec / np.max(st[i].data))) -
...
def get_CorrStuff(st):
    ...
    for i in xrange(ntr):
        ...
        ipeaks = find_peaks_cwt(cor_smooth2, np.arange(1, len(cor_smooth2)/2))
        n_peaks = 0
        for ip in ipeaks:
...
def get_freq_band_stuff(st):
    ...
    for i in xrange(ntr):
        for j in xrange(nf):
            ...
            data_filt = l2filter(Fb, Fa, tr.data)
...
def get_full_spectrum_stuff(st):
    ...
    for i in xrange(ntr):
        ...
        ipeaks = find_peaks_cwt(FFTsmooth_norm,
                                np.arange(1, len(FFTsmooth_norm)/2))
